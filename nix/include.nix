# Generates headers from the accounts module plugin using logos-cpp-generator
...
pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    if [ -f "${lib}/lib/accounts_module_plugin.dylib" ]; then
      PLUGIN_FILE="${lib}/lib/accounts_module_plugin.dylib"
    elif [ -f "${lib}/lib/accounts_module_plugin.so" ]; then
      PLUGIN_FILE="${lib}/lib/accounts_module_plugin.so"
    else
      echo "Error: No wallet_accounts_plugin library file found"
      exit 1
    fi
    ...
    logos-cpp-generator "$PLUGIN_FILE" --output-dir ./generated_headers --module-only || {
      echo "Warning: logos-cpp-generator failed, this may be expected if the accounts module has no public API"
